...
int main(void)
{
	...
	while (1) {
		...
		if (input_flag) {
			...
			/* Did we change the waveform type? */
			if (wave_type_update) {
				wave_type = input_get_sel_wave();
				display_set_wavetype(wave_type);
				function_gen_set_type(wave_type);
			...
			/* Did we change the modifier value? */
			if (modifier_update) {
				display_set_modifier(input_get_modifier());
				modifier_update = 0;
			}

			/* Did we change the dutycycle? */
		...
		/*
		 * Time handler
		 * Measures approx. 100ms to set the input flag
		 */
		time_handler();
	}
}
